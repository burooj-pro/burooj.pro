...
const pipedriveFormUrl = config.public.pipedriveFormUrl as string
const pipedriveLoaderSrc = config.public.pipedriveLoaderSrc as string
...
onMounted(() => {
  ...
  const existing = document.querySelector(`script[src="${pipedriveLoaderSrc}"]`)
  if (existing) {
  ...
  script.async = true
  script.onload = () => { formLoading.value = false }
  script.onerror = () => { formLoading.value = false; formLoadError.value = true }
  ...
  <section ref="contactSection" class="section-wrapper relative z-10 pt-32 pb-16 md:pt-40 md:pb-24">
    ...
    <div class="grid gap-12 lg:grid-cols-[1fr_1.2fr] lg:gap-16">
          ...
          <div class="mt-8">
            ...
            <div class="flex items-center gap-4">
              ...
              <a
                ...
                rel="noopener noreferrer"
                class="text-slate-600 transition-colors hover:text-primary"
                aria-label="WhatsApp (+966 53 555 8889)"
              >
                <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      ...
      <div ref="contactForm" class="space-y-4">
        <ClientOnly>
          <!-- Skeleton shown while Pipedrive script loads -->
          <div v-if="formLoading" class="animate-pulse space-y-4 rounded-lg border border-slate-200 p-6">
          ...
          <!-- Pipedrive error fallback -->
          <div v-else-if="formLoadError" class="rounded-lg border border-slate-200 bg-slate-50 p-8 text-center">
            <p class="mb-4 text-sm text-slate-600">The contact form could not be loaded.</p>
            <a
              :href="pipedriveFormUrl"
              target="_blank"
              rel="noopener noreferrer"
              class="inline-block rounded-md bg-primary px-6 py-3 text-sm font-medium text-white transition-colors hover:bg-primary-dark"
            >
              Open form in new tab
            </a>
          </div>
          <div v-else class="pipedriveWebForms w-full max-w-[720px]" :data-pd-webforms="pipedriveFormUrl" />
          <template #fallback>
            <div class="animate-pulse space-y-4 rounded-lg border border-slate-200 p-6">
              <div class="h-10 rounded bg-slate-100" />
              <div class="h-10 rounded bg-slate-100" />
              <div class="h-10 rounded bg-slate-100" />
              <div class="h-24 rounded bg-slate-100" />
